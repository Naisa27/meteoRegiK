...
from schemas.importResult import ImportResult
from schemas.sourceData_rp5 import SourceDataRp5Add
...
class DataImport(Database):
    ...
    def import_from_excel(self, filepath: str, data_format: str, progress_callback = None) -> ImportResult | None:
        try:
            df = None
            if data_format == "rp5":
                ...
                """
                будет парсер екселя формата rgm
                """
            else:
                ...
                return ImportResult(
                    success=False,
                    errors=[f"Неизвестный формат: {data_format}. Распарсить невозможно."]
                )
            ...
            if df.empty:
                ...
                return ImportResult(
                    success=False,
                    errors=[f"Файл пуст или не содержит данных"]
                )
            ...
            if data_format == "rp5":
                ...
                return ImportResult(
                    success=False,
                    errors=[f"Пока еще обработчик не написан"]
                )
            ...
            import_result = ImportResult(
                success=False,
                errors=[f"Не удалось загрузить данные:\n{str( e )}"]
            )
            print(import_result)
    ...
    def _import_rp5_to_sql( self, df: pd.DataFrame, data_format: str, progress_callback = None ) -> ImportResult:
        """
        ...
        if total_rows == 0:
            message = f"Файл пуст. нет данных для заливки"
            messagebox.showinfo( title="Импрот даже не начинался", message=message )
        ...
        with DBManager(self._session_factory) as db:
            ...
            if len( data_exists ) == total_rows:
                message = f"Все строки в количестве {len( data_exists )} уже существуют в БД"
                messagebox.showinfo( title="Импрот даже не начинался", message=message )
                errors.append( message )
                import_result = ImportResult(
                    success=False,
            ...
            for ind, row in df.iterrows():
                try:
                    ...
                    data = SourceDataRp5Add(
                        ...
                        Tg_min_temperature_surface_soil=self.clean_excel_value(row.get( "Tg" )),
                        E1_surface_soil_full=self.get_str_value(row.get( "E'" )),
                        sss_height_snow=self.get_str_value(row.get( "sss" ))
                    )
                    if  [ item for item in data_exists if item.dt == data.dt and item.time_point == data.time_point ]:
        ...
        """
        messagebox.showinfo(title="Импрот завершён",message=message)
